...
function harvestCodex(taskdir, startMs, agent) {
  ...
  let offset = 0;
  let mcpUsage = null;
  for (const rawEv of readJsonLines(path.join(taskdir, "events.jsonl"))) {
    const ev = rawEv.type === "codex/event" && rawEv.event ? rawEv.event : rawEv;
    const ts = eventTs(ev, startMs + (++offset));
    const msg = ev.msg || {};
    const item = ev.item || {};
    if (rawEv.type === "mcp.tool.result") {
      events.push({ ts, type: "message.out", message: String(rawEv.content || ""), agent });
    } else if (msg.type === "agent_message") {
      events.push({ ts, type: "message.out", message: String(msg.message || ""), agent });
    } else if (msg.type === "token_count") {
      mcpUsage = { ts, type: "llm.usage",
        ...normalizeUsage((msg.info && (msg.info.total_token_usage || msg.info.last_token_usage)) || {}), agent };
    } else if (msg.type === "task_complete") {
      events.push({ ts, type: "turn.completed", agent });
    } else if (ev.type === "item.completed" && item.type === "command_execution") {
      events.push({
        ts,
        type: "command",
  ...
  if (mcpUsage) events.push(mcpUsage);
