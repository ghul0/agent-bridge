...
function runProcessTransport({ agent, spec, prompt, full, runCwd, sandbox, taskdir, dir, id,
  transport, worktree, branch, pr, session, sess, statusFile, cwd, model }) {
  const resuming = !!(sess && sess.sessionId && spec.resumeBuild);
  const { args, opts } = resuming
    ? spec.resumeBuild({ sessionId: sess.sessionId, prompt: full, cwd: runCwd, sandbox, taskdir, model })
    : spec.build({ prompt: full, cwd: runCwd, sandbox, taskdir, model });
...
function dispatch({ agent, prompt, cwd, sandbox, isolate, pr, session, transport, model }) {
  ...
  return runProcessTransport({ agent, spec, prompt, full, runCwd, sandbox, taskdir: dir,
    dir, id, transport, worktree, branch, pr, session, sess, statusFile, cwd, model });
}
