...
const AGENTS = {
  codex: {
    ...
    build({ prompt, cwd, sandbox, taskdir, model, agentProfile }) {
      const sb = sandbox === "read-only" ? "read-only" : "workspace-write";
      const m = model ? ["-m", model] : [];
      const ap = agentProfile ? ["-p", agentProfile] : [];
      return {
        args: ["exec", ...m, ...ap, "--json", "--sandbox", sb, "-C", cwd, "--skip-git-repo-check",
          "--add-dir", taskdir, prompt],
        opts: { stdio: ["ignore", "pipe", "pipe"] },
      };
    },
    resumeBuild({ sessionId, prompt, cwd, model, agentProfile }) {
      ...
      const ap = agentProfile ? ["-p", agentProfile] : [];
      return {
        args: ["exec", "resume", ...m, ...ap, "--json", "--skip-git-repo-check", sessionId, prompt],
  ...
  antigravity: {
    ...
    selfStatus: false,
    build({ prompt, cwd, sandbox, model, agentProfile }) {
      ...
      const ap = agentProfile ? ["--agent", agentProfile] : [];
      return {
        args: ["-p", prompt, ...m, ...mode, ...ap, "--add-dir", cwd],
        opts: { cwd, stdio: ["ignore", "pipe", "pipe"] },
      };
    },
    resumeBuild({ sessionId, prompt, cwd, sandbox, agentProfile }) {
      const mode = sandbox === "read-only" ? ["--mode", "plan"] : ["--dangerously-skip-permissions"];
      const ap = agentProfile ? ["--agent", agentProfile] : [];
      return {
        args: ["--conversation", sessionId, "-p", prompt, ...mode, ...ap, "--add-dir", cwd],
        opts: { cwd, stdio: ["ignore", "pipe", "pipe"] },
      };
    },
